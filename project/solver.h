...
public:
    ...
    Solution Solve(const std::unordered_set<Point, PointHash>& p1, const std::unordered_set<Point, PointHash>& p2, const std::unordered_set<Point, PointHash>& p3, int board[8][8], bool useCache = true);
...
private:
    ...
    int maxPossibleScore(int board[8][8]);
    size_t hashBoard(int board[8][8]);

    int maxResult = 0;
    int statesEvaluated = 0;
    int cacheHits = 0;
    bool cachingEnabled = true;
    std::unordered_map<size_t, int> blastCache;
